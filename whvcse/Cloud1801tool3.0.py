...
print('                                                       |___/ |___/             ')
print('说明：本程序为交流学习使用，禁止外传，禁止用于违规用途，禁止商业用途;\n若因使用本程序造成违法行为，本人概不负责')
...
while y < class_num:
    ...
    if class_success=='未通过':
        YN=input("说明：仅当前学期有效，请不要选择已过期课程,确认即输入‘y’后回车，反之直接回车即可；\n是否要刷<"+str(class_name)+">这门课（y/n）：")
        if YN=='y':
...
while negriuwg < class_num:
    ...
    if class_success=='未通过':
        YN = input("说明：仅当前学期有效，请不要选择已过期课程,确认即输入‘y’后回车，反之直接回车即可；\n是否要开始<" + str(class_name) + ">这门课的考试（y/n）：")
        if YN == 'y':
